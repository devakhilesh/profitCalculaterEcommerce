...
if (!razorpay_payment_id) {
  razorpay_payment_id = payment.paymentInfo && payment.paymentInfo.razorpay_payment_id;
}
...
if (razorpay_order_id && razorpay_payment_id && razorpay_signature) {
  ...
  try {
    ...
    console.warn("Warning: failed to fetch payment from Razorpay:", err.message);
  }
...
if (razorpay_payment_id) {
  ...
  const dbOrderId = payment.paymentInfo && payment.paymentInfo.razorpay_order_id;
  ...
  if (dbOrderId && rpOrderId && dbOrderId !== rpOrderId) {
    ...
    return res.status(400).json({
      status: false,
      message: "Payment order mismatch (Razorpay payment does not belong to this order/payment record)",
    });
  }
