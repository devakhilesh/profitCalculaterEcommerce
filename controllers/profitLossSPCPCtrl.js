...
const SellingPriceCalculationModel = require("../models/sellingCalcModel");
...
exports.getAllProfitHistory = async (req, res) => {
  try {
    ...
    const getAll = await productSPCP_ProfitLossCalcModel
      .find({
        userId: req.user._id,
...
exports.createSellingHistory = async (req, res) => {
  try {
    ...
    const saveData = await SellingPriceCalculationModel.create({
      ...req.body,
      platformId,
    });
...
exports.getAllSellingHistory = async (req, res) => {
  try {
    ...
    const getAll = await SellingPriceCalculationModel.find({
      userId: req.user._id,
      platformId: platformId,
    }).sort({ updatedAt: -1 });
